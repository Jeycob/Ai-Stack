...
Viditelná odpověď z agent loopu je záměrně lidská: nahoře je krátký výsledek, potom maximálně několik stavových řádků a až dole debug block s markery jako `AGENT_LOOP_OK`, `workflow=...`, raw `plan`, `taskspec` a `execution`. Když se mění soubory, odpověď vypíše konkrétní changed files a verify status. Self-improve transcript normalizer navíc čte assistant text nejen z `content`, ale i z OpenAI/OpenWebUI variant jako `output[].content[].text`; prázdný nebo nenačtený incident končí `TRANSCRIPT_FETCH_FAILED` nebo `TRANSCRIPT_EMPTY`, nikdy falešným “unknown OK”.
