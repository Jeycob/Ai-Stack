...
Planner uvnitř gateway je nově výslovně `LLM-first`. To znamená, že `admin_agent_loop()` se nejdřív vždy pokusí získat plán od modelu a teprve když planner call selže nebo vrátí nepoužitelný výstup, přejde na malý bounded fallback pro strukturální vstupy jako explicitní URL, Git remote URL, explicitní příkaz v backticku nebo read-only safety guard. Běžná čeština/angličtina se ve fallbacku neinterpretuje; když je Ollama/LLM backend vypnutý, gateway raději vrátí jasný `clarify` s recovery než aby hádala podle keywordů. V audit výstupu je to vidět přes `planner_source=llm|fallback` a `routing_provenance`, takže už se dá snadno poznat, jestli systém opravdu rozhodoval přes model, nebo jen zachraňoval běh bez něj.
...
Meta dotazy jsou řešené deterministicky, ne přes obecný repo review: `workspace_context_set`, `workspace_context_status`, `capability_catalog_show` a `agent_runtime_status` vrací aktuální workspace, známé workspaces, capability katalog nebo runtime stav. Viditelná odpověď má být lidská a krátká; například capability help nejdřív shrne hlavní oblasti (`chat`, `web`, `workspace`, `self-improve/deploy`) a teprve technické aliasy nebo plný katalog nechá v debug/detail vrstvě. Dotaz typu “prohledej repo a hledej capability” jde do `workspace_search`, což je bounded `rg` přes workspace s ignorováním runtime/secrets adresářů; pokud by search capability chyběla nebo neměla query, gateway vrátí konkrétní `NEEDS_ATTENTION` místo halucinovaného shrnutí.
