...
Stejnou logiku teď zná i `mentor_codex_local.py`: `delegate`, `profile`, `report`, `plan` i `next-helper` rozlišují mezi `push-check`, `push` a `release boundary`, takže codex-local nepůsobí nekonzistentně mezi helper vrstvou a auto-tools routováním.
