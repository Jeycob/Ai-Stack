...
- `codex/bin/openwebui_codex_auto_tools_filter.py`: tenká OpenWebUI bridge vrstva pro `codex-local`; běžný jazyk neinterpretuje jako workflow, ale předává ho do gateway agent loopu.
...
Primární způsob práce je přes viditelný OpenWebUI audit chat. Instrukce mají být konkrétní, ideálně s prvním řádkem `repo: <workspace>`, aby gateway měla jasný controller/target workspace. Přirozený jazyk se v OpenWebUI filtru nemá překládat na konkrétní workflow; thin filter ho předá do `GATEWAY_ADMIN_AGENT_LOOP` a workflow určí LLM TaskSpec planner.
Explicitní workspace/file prefixy jako `repo:`, `workspace:`, `projekt:`, `soubor:`, `file:` nebo `path:` jsou strukturální metadata, ne business router. Slouží k určení kontextu a povoleného souboru, zatímco význam úlohy pořád patří do TaskSpecu.
...
Gateway soubor načte přímo z registrovaného workspace, blokuje runtime/secrets cesty jako `.env`, `codex/state/` a `codex/audit/`, a lokální model pak odpoví z reálného očíslovaného obsahu souboru. Rozhodnutí, že jde o vysvětlení souboru, má vzniknout v TaskSpec/capability vrstvě, ne v seznamu českých frází ve filtru.
...
V codex-local cestě se tento prompt neposílá přes pevný překlad `vytvoř repo -> konkrétní admin command`. Thin filter ho pošle do agent loopu, LLM planner vytvoří TaskSpec s cílem `workspace_repo_bootstrap` a capability runner teprve potom auditovaně vytvoří lokální repo pod `/mnt/c/Repositories`, inicializuje Git, přidá README, vygeneruje private key do ignorovaného `codex/state/ssh/`, vrátí public key a zaregistruje workspace. Nerestartuje stack a nic nepushuje, pokud o to TaskSpec/policy výslovně nepožádá.

Pokud zadání obsahuje i follow-through cíl jako inicializace Gitu, SSH key, GitHub remote, install, smoke nebo push, má to být jeden cílový end-state v TaskSpecu a capability runner má zastavit až na prvním skutečně externím checkpointu. Tohle není místo pro další seznam prompt frází v gateway core.
...
Pokud TaskSpec požaduje GitHub repo a je dostupný `GITHUB_TOKEN`, `GITHUB_TOKEN_FILE`, nebo ignorovaný `codex/state/github-api.token`, capability runner může vytvořit GitHub repo, přidat public key jako write deploy key a nastavit lokálnímu repozitáři `origin`. Bez tokenu vrátí jasný `GITHUB_TOKEN_MISSING`; nebude tvrdit, že GitHub repo vzniklo.
...
TaskSpec planner to má vyhodnotit jako `workspace_run` a capability runner z toho sestaví auditovaný workspace příkaz. Ruční admin ekvivalent pro diagnostiku je:
...
U nejběžnějších read-only repo akcí není nutné psát ani `spusť příkaz:`. Planner má pochopit cíl, vybrat `workspace_run` nebo specializovanou read-only capability a runner pak spustí bounded příkaz v kontejneru. Pokud planner není dostupný, běžná věta se nemá hádat podle keywordů; fallback přijímá jen strukturální příkaz v backticku nebo explicitní admin command.
...
TaskSpec planner z toho má vybrat `workspace_action:install` nebo `workspace_action:test`; explicitní admin ekvivalent pro diagnostiku je:
...
Deploy flow po restartu vždy spouští stejný runtime fingerprint gate z runtime checkoutu. Pokud `/health` hlásí nový commit, ale běžící gateway proces je ještě starý, deploy skončí markerem `DEPLOY_BLOCKED_GATEWAY_RUNTIME_DRIFT` a `GATEWAY_ADMIN_DEPLOY_STATUS` vrátí konkrétní `runtime_gate` payload plus `restart_required=True`.
...
Codex-local cesta má z promptu přes TaskSpec zvolit `public_web_search`, `web_fetch` nebo `web_answer`. Gateway pak stáhne jen veřejný HTTP/HTTPS zdroj přes omezený GET, blokuje lokální a privátní adresy, nepoužívá cookies ani secrets, z HTML vytáhne čitelný text a lokální model odpoví pouze z načteného zdroje. Ruční admin ekvivalenty jako `GATEWAY_ADMIN_WEB_ANSWER <url> -- <otázka>` nebo `GATEWAY_ADMIN_WEB_FETCH <url>` existují pro diagnostiku, ne jako běžný keyword router.

Follow-up typu “vypiš výsledky” má navázat přes vyčištěný conversation context a `referents` v TaskSpecu. Pokud planner není dostupný, gateway nesmí vymýšlet veřejné hledání podle české fráze; vrátí konkrétní recovery/clarify.
...
Planner uvnitř gateway je nově výslovně `LLM-first`. To znamená, že `admin_agent_loop()` se nejdřív vždy pokusí získat plán od modelu a teprve když planner call selže nebo vrátí nepoužitelný výstup, přejde na malý bounded fallback pro strukturální vstupy jako explicitní URL, Git remote URL, explicitní příkaz v backticku, přesný allowlisted command nebo explicitní meta/admin tvar. Běžná čeština/angličtina se ve fallbacku neinterpretuje; když je Ollama/LLM backend vypnutý, gateway raději vrátí jasný `clarify` s recovery než aby hádala podle keywordů. V audit výstupu je to vidět přes `planner_source=llm|fallback` a `routing_provenance` (`llm_taskspec`, `capability_canonicalizer`, `policy_guard`, `runtime_guard`, `fallback:structural`, `fallback:planner_offline`, `manual_admin_command`), takže už se dá snadno poznat, jestli systém opravdu rozhodoval přes model, nebo jen zachraňoval běh bez něj.

Fallback ani TaskSpec normalizer už nevybírají meta/help/search/ssh/deploy/edit workflow podle českých nebo anglických vět. Tyto záměry musí přijít jako `intent_class`, canonical capability nebo explicitní strukturované pole v TaskSpecu; bez toho gateway vrátí clarify/recovery místo tiché náhrady jinou akcí.

Stejně tak starý `normalize_agent_plan()` už neslouží jako slovníkový router. Zůstává jen jako compatibility normalizer pro explicitní plán nebo strukturální fallback payload: ořeže pole, canonicalizuje capability a případně doplní konkrétní URL, Git remote nebo backticked command, ale nedopočítává `review`, `bootstrap`, `ssh`, `edit`, `deploy` ani workspace action z volného textu. Offline guard `gateway_llm_first_guard_smoke.py` failne, pokud se do této vrstvy vrátí prose compatibility hook.
...
TaskSpec v2 přidává i `intent_class`, `referents`, `target_workspace`, `target_capability_name`, `execution_plan`, `needs_user_input` a `answer_visibility`. Díky tomu se běžné chat dotazy typu `kolik je 10+52`, `co je dnes za den?` nebo `write me a story` routují na `direct_answer`/`creative_answer`, ne do repo review; odpověď ale pořád generuje model z předaného runtime/contextu, ne gateway přes pevné české fráze. Dotazy typu `jaké máš capability?` jdou na `capability_help`, veřejné hledání bez konkrétní URL na `public_web_search` a širší app flow typu `vytvoř appku, nainstaluj, buildni, otestuj a vystav preview` na `workspace_action_chain`. Follow-up výrazy jako `to`, `tam` nebo `ten projekt` patří do `referents`, takže model má explicitní místo, kam ukládat vyřešený kontext místo dalšího prompt-specific routeru. Gateway teď navíc TaskSpec planneru předává krátký redigovaný kontext posledních zpráv, takže follow-up jako `vyhledej to kdekoliv` může vyřešit “to” z předchozího dotazu bez dalšího keyword routeru. Pokud chain obsahuje preview/expose cíl a `smoke` krok z výstupu odhalí lokální URL typu `http://127.0.0.1:4173`, autopilot ji vytáhne do lidské odpovědi jako nalezený preview endpoint místo obecného “autopilot doběhl”.

Kontext pro planner se před odesláním čistí přes `recent_conversation_context`: technické wrappery typu `GATEWAY_ADMIN_AGENT_LOOP`, raw `CODEX_DEBUG`, `model_runtime`, `workflow=` a execution packety se vyhodí, ale skutečné user cíle a stručná assistant shrnutí zůstávají. Tím se follow-up typu `vypiš výsledky` nebo `pokračuj` může opřít o historii bez toho, aby LLM dostalo zpět vlastní runtime logy jako “uživatelský” prompt.
...
Capability runner má mít u každé implementované capability explicitní contract: `capability_id`, workflow, input schema, output schema, safety profile, executor, recovery a testy. `/health` proto vrací `capability_registry.contract_issues` a `gateway_recovery_smoke.py` failne, pokud implementovaná capability nemá aspoň minimální kontrakt. Smysl je jednoduchý: žádný tichý fallback do jiné akce, žádné “nějak to zkusím”; buď capability splní svůj kontrakt, nebo vrátí přesný blocker a další bezpečný krok.

Jakmile TaskSpec zvolí meta capability, její executor je deterministický a nejde přes obecný repo review: `workspace_context_set`, `workspace_context_status`, `capability_catalog_show` a `agent_runtime_status` vrací aktuální workspace, známé workspaces, capability katalog nebo runtime stav. Viditelná odpověď má být lidská a krátká; například capability help nejdřív shrne hlavní oblasti (`chat`, `web`, `workspace`, `self-improve/deploy`) a teprve technické aliasy nebo plný katalog nechá v debug/detail vrstvě. Dotaz typu “prohledej repo a hledej capability” má TaskSpec zvolit jako `workspace_search`, což je bounded `rg` přes workspace s ignorováním runtime/secrets adresářů; pokud by search capability chyběla nebo neměla query, gateway vrátí konkrétní `NEEDS_ATTENTION` místo halucinovaného shrnutí.

Viditelná odpověď z agent loopu je záměrně lidská: nahoře je krátký výsledek a potom maximálně několik stavových řádků. Strojové markery jako `AGENT_LOOP_OK`, `workflow=...`, raw `plan`, `taskspec` a `execution` zůstávají v obsahu pro helpery, ale defaultně jsou schované v `CODEX_DEBUG` HTML comment bloku; viditelně je zobrazíš přes `CODEX_LOCAL_SHOW_DEBUG=1` nebo `answer_visibility=details`. Když se mění soubory, odpověď vypíše konkrétní changed files a verify status. Self-improve transcript normalizer navíc čte assistant text nejen z `content`, ale i z OpenAI/OpenWebUI variant jako `output[].content[].text`; prázdný nebo nenačtený incident končí `TRANSCRIPT_FETCH_FAILED` nebo `TRANSCRIPT_EMPTY`, nikdy falešným “unknown OK”.
...
Od commitu `7cb415e` a navazujících změn je navíc `TaskSpec` plán považovaný za capability-locked. Prakticky to znamená, že když LLM planner zvolí například `workspace_git_publish`, `workspace_action:verify` nebo read-only `review`, následná normalizační vrstva už ten workflow nesmí znovu přepsat jen proto, že se v promptu objevují slova jako `repo`, `ssh`, `github`, `push`, `oprav` nebo `spusť`. Staré keyword heuristiky byly z gateway core odstraněné; bez použitelného TaskSpecu se má vrátit konkrétní `NEEDS_ATTENTION`/`clarify`, ne jiná akce. Offline guard `codex/bin/gateway_llm_first_guard_smoke.py` hlídá, že se do core nevrátí české/anglické cue listy pro business routing. Druhý guard `codex/bin/openwebui_filter_llm_first_smoke.py` ověřuje OpenWebUI filtr: každý `codex-local` prompt se má jen obalit do `GATEWAY_ADMIN_AGENT_LOOP`, nikoli předem přeložit na konkrétní admin workflow jako SSH keygen, deploy nebo workspace action.

Zbytky historického keyword routingu jsou vedené explicitně v `docs/codex-local-keyword-routing-inventory.json`. Tenhle soubor není povolenka rozšiřovat starou cestu; je to auditní mapa, která rozlišuje `core_llm_first`, tenký OpenWebUI filter s legacy helpery, starý `mentor_codex_local.py` helper a self-improve recovery fallback. `codex/bin/keyword_routing_inventory_smoke.py` kontroluje, že tyto plochy jsou deklarované, že gateway core má svůj LLM-first guard, že OpenWebUI filter má svůj thin-filter guard a že legacy helpery zůstávají pojmenovaný dluh. `check_ai_stack.sh` tenhle smoke spouští spolu s ostatními levnými guardy.
...
Základní workspace capability akce jsou centralizované v `docs/codex-local-capability-roadmap.json` pod `workspace_actions`: `install`, `test`, `build`, `lint`, `verify` a `smoke`, včetně timeoutů, runneru a recovery signatur. Přirozený jazyk k těmto akcím patří do LLM TaskSpec planneru, prompt šablon a regresních scénářů; gateway core z roadmapy nemá dělat slovníkový router.
...
`target_capability_name` je součást TaskSpec kontraktu. Planner tak nemusí schovávat název nové capability do volného textu; gateway ho předá do self-improve helperu a helper podle něj vytvoří `generated-unified.diff`. Když planner jméno capability vynechá, gateway core ho z přirozené věty nedopočítává: vrátí `clarify`/`NEEDS_ATTENTION`, protože název capability je sémantické rozhodnutí planneru, ne keyword parseru. Vzniklý diff je omezený na povolené cesty, typicky `docs/`, `codex/bin/`, `codex/gateway/` a kořenové provozní soubory, a ještě před apply musí projít `git apply --check`. Nově `generate_unified_diff` nejdřív zkusí LLM-first executor draft: přes stejné gateway runtime helpery jako hlavní agent požádá codex-local o jeden bounded unified diff pouze pro povolené cesty. Když tenhle draft projde `extract_unified_diff` a `git apply --check`, helper ho u capability-develop toku uloží jako samostatný review artifact `llm-review-draft.diff`; když ne, helper to explicitně zapíše do `llm_attempt` v `generated-diff-result.json` a spadne zpět na deterministický capability/failure template bundle. Self-improve tak dostává reálnější patch-preparing chování, ale pořád bez rozbití guard rails nebo tichého auto-apply. Capability draft už netvoří jen roadmap poznámku, ale i strojově validovatelný smoke kontrakt `docs/capability-drafts/<capability>.smoke.json`, konkrétní gateway integration draft `docs/capability-drafts/<capability>.gateway-integration.json`, review-friendly gateway patch fragment `docs/capability-drafts/<capability>.gateway.patch.md`, samostatný runtime patch candidate `docs/capability-drafts/<capability>.runtime.patch.diff`, nový review-only promotion patch `docs/capability-drafts/<capability>.promotion.patch.diff`, wiring blueprint `docs/capability-drafts/<capability>.wiring.json`, explicitní executor contract `docs/capability-drafts/<capability>.executor-contract.json`, konkrétní executor dispatch plán `docs/capability-drafts/<capability>.executor-dispatch.json`, implementační work order `docs/capability-drafts/<capability>.implementation-workorder.json` a sadu Python scaffoldů pod `codex/bin/capability_drafts/`: executor stub, runtime hook stub a smoke stub. Gateway integration draft nese přenositelné snippet návrhy pro `AGENT_CAPABILITY_TO_WORKFLOW`, `CANONICAL_AGENT_CAPABILITY_ALIASES`, registry entry, TaskSpec binding a executor hook; gateway patch fragment z toho navíc skládá čitelný diff-like draft přesně pro `gateway.py`, takže senior review nemusí skládat změnu z několika JSON souborů ručně. Runtime patch candidate jde ještě o kus dál: ukládá samostatný unified-diff-like návrh přímo proti `codex/gateway/gateway.py`, včetně konkrétního `run_<capability>_capability(...)` handler shape, dispatch selectoru a payloadu pro executor stub, pořád ale jen jako audit artifact, ne jako auto-apply vstup. Promotion patch jde ještě o krok dál a skládá skutečný review-ready unified diff proti aktuálnímu `gateway.py`: doplní workflow mapu, `CORE_AGENT_CAPABILITIES` i canonical aliasy pro draft capability. Pořád zůstává review-only, ale už je to mnohem použitelnější podklad pro senior Codex review než jen slovní fragment. Executor contract mezitím popisuje konkrétní vstupy, preconditions, návratový kontrakt a recovery pravidla capability, zatímco executor dispatch plán určuje přesný helper module, entrypoint a verify příkazy. Implementační work order nad tím skládá konkrétní codex-local kroky, artefakty ke čtení, review checkpointy a escalation points, takže další krok tak není jen „dopsat kód nějak“, ale implementovat přesně definovaný bounded executor + dispatcher shape podle explicitního pracovního balíku. Runtime hook stub pak zachytí konkrétní registry/alias/workflow shape ještě předtím, než by se sahalo do živého `gateway.py`. Wiring blueprint vypíše touchpointy v gateway/filter/test vrstvě, implementační kroky, review split mezi codex-local a senior Codexem a recovery pravidla, takže z diffu neleze jen dokumentace, ale přímo auditovatelný code-like patch draft pro další implementační krok. `patch-proposal.json` už navíc obsahuje i konkrétní `proposed_file_changes`, acceptance criteria, offload split a guarded unified-diff expectations. Nově k nim přidává i `acceptance_evidence_plan`: pro každé acceptance criterion ukáže, které draft artefakty a které verify příkazy mají sloužit jako důkaz. Finální `self-improve-report.json` a `self-improve-report.md` pak nad tím dopočítají `acceptance_evidence_status`, takže je vidět nejen co se mělo vytvořit, ale i které evidence body jsou skutečně pokryté vygenerovaným diffem a verify sadou. `capability_patch_readiness` už navíc nehlídá jen existenci draft artefaktů, ale i to, že verify sada je zelená a že acceptance evidence status nemá díry. Stejný signál se propisuje i do `guarded-apply-manifest.json`: ten teď nese `verify_all_green`, `missing_acceptance_evidence` a používá je v `promotion_blockers` i `promotion_ready`. Report a manifest nově explicitně nesou i `runtime_gate_status` pro `deploy` a `e2e`, takže je přímo v artefaktech vidět, jestli byl live runtime gate jen připravený, opravdu prošel, nebo zablokoval další krok kvůli markeru typu fingerprint/source-epoch drift. Codex-local tak nevrací jen checklist, ale i skutečný malý implementační plán po souborech, důkazní mapu k němu a tvrdší readiness/promotion signál. Pro běžné menší fail/fix případy bez nové capability helper už nespadne na `no_safe_generator`, ale generuje bezpečný failure regression bundle pod `docs/self-improve-cases/`: case JSON, smoke kontrakt, patch fragment, runtime patch candidate a Python smoke scaffold. Tím dostaneš reálný unified diff i pro menší recovery/feature změny, aniž by self-improve musel rovnou upravovat živý runtime. Nově z `generated-unified.diff` vznikají i dvě praktické odnože: `safe-apply-candidate.diff`, která automaticky obsahuje jen safe apply kandidátní části, a `runtime-review-only.diff`, která oddělí review-only runtime/gateway artefakty. Codex-local tak odpracuje i mechanické rozdělení draftu a senior Codex už neřeší ruční vysekávání patchů z jednoho velkého diffu. Nad tím vším teď vzniká i `guarded-apply-manifest.json`, který explicitně rozdělí safe apply candidate paths a runtime review-only artifacty, vypíše minimum verify příkazů, `promotable_runtime_candidates`, `promotion_blockers` a `promotion_ready`, a připomene, že runtime patch candidate se nikdy nesmí auto-applynout bez senior review a fingerprint gate. Guarded patch fáze navíc už defaultně nebere celý `generated-unified.diff`, ale právě `safe-apply-candidate.diff`; review-only bundle tak zůstává mimo automatický apply path i tehdy, když self-improve běží v patch/full režimu. Když verify nebo guarded patch selže a běží `max_cycles > 1`, další iterace už navíc nedostává jen obecný blocker, ale konkrétní `repair_context`: seznam failnutých fází, jejich výstupní tails, `git apply --check` diagnostiku, failing verify commands a cestu k předchozím artifactům. Díky tomu se druhý cycle může opřít o reálný failure snapshot místo slepého opakování stejného návrhu. K běhu navíc vzniká finální `self-improve-report.json` a `self-improve-report.md`, kde je pohromadě offload split, phase status, runtime gate status, verify summary, capability patch readiness, generated artifacts, patch decision a důvod, proč se patch aplikoval nebo zůstal jen ve validated-only režimu. `patch --dry-run` nad tímto generated diffem je také součást smoke sady, takže guarded apply path není jen deklarovaný, ale i přímo testovaný proti `git apply --check`. Defaultní OpenWebUI agent loop spouští self-improve jako `dry_run=True`; skutečný apply má jít přes auditovaný admin/CLI krok po senior review.
...
`agent_capability_develop` je samostatná capability v katalogu. Nemá přidávat další prompt-specific keyword router; má vytvořit TaskSpec/acceptance criteria, registry změny, executor/workflow scope, roadmap/docs, testovací plán a unified diff draft. Nově i samotná `reason` fáze v `agent_self_improve.py` běží LLM-first: zkusí přes stejné gateway structured-output helpery jako hlavní agent vytvořit TaskSpec planner JSON, pak ho protáhne existující gateway canonicalization/normalization vrstvou a teprve potom ho enrichne o `repair_context` a acceptance criteria pro self-improve. Když planner backend není dostupný nebo selže, helper to explicitně přizná (`structured_taskspec_runtime_fallback`) a spadne zpět na deterministic fallback místo tichého keyword routingu. Offline to hlídá `codex/bin/agent_self_improve_reasoning_smoke.py`. Codex-local tím může odpracovat rutinní průzkum, návrh testů, patch proposal, diff draft, smoke běhy a recovery report, zatímco senior Codex drží finální architekturu, bezpečnostní hranice a review aplikovaného diffu.

Live runtime fingerprint check už není přivázaný jen na `127.0.0.1:9101`. `gateway_runtime_fingerprint_check.py` nově zkouší kandidáty v pořadí explicitní `--base-url`, `CODEX_GATEWAY_URL`, `CODEX_GATEWAY_PUBLIC_URL`, compose-derived LAN URL a teprve pak localhost fallback. Díky tomu se check lépe chová i ve WSL/LAN splitu, kde běžící gateway není dostupná na loopbacku, ale je vidět přes `192.168.x.x:9101`. Offline to kryje `codex/bin/gateway_runtime_fingerprint_check_smoke.py`; `check_ai_stack.sh` ho nově pouští vedle live fingerprint gate, takže se lépe odliší „runtime není dostupný vůbec“ od „runtime checker jen zvolil špatný endpoint“.

Stejným směrem nově jde i samotná diagnóza failu. `agent_self_improve` si nejdřív zkusí přes structured-output runtime vyžádat bounded diagnosis JSON: category, root cause, patch scope a recovery. Deterministická vrstva pak už nedělá plnohodnotný routing, jen hlídá guard rails: category musí spadat do známého katalogu, `patch_scope` se ořeže na povolené ai-stack cesty a při chybě planner backendu se helper vrátí na `structured_diagnosis_runtime_fallback`. Tím se další kus práce přesouvá z ručně psaných markerů do LLM reasoning vrstvy, ale bez ztráty fail-closed chování.

Stejný princip nově platí i pro regression planning. `agent_self_improve` si zkusí nechat navrhnout regression cases přes structured-output runtime, ale výsledné case payloady ještě normalizuje: doplní bezpečný slug, zachová bounded pole (`prompt`, `expected_workflow`, `expected_capability`, `expected_marker`) a při pádu planner backendu se vrátí na `structured_regression_runtime_fallback`. Self-improve tak umí offloadnout i návrh regresních scénářů, aniž by se repo vrátilo k ručně rozlezlému prompt pattern matchingu.

Nově je LLM-first i `propose_patch`. Runtime planner může vrátit vlastní seznam bezpečných file changes, acceptance criteria a offload split, ale deterministická vrstva je ještě před zápisem do artefaktu canonicalizuje: vyhodí zakázané cesty jako `.env`, zachová fallback acceptance criteria z TaskSpecu a doplní chybějící bounded file plans z deterministického capability/failure bundle. Self-improve tak přesouvá i rutinní patch-planning práci na codex-local, ale pořád fail-closed a bez tichého rozšíření patch scope.

Capability promotion draft už navíc není jen registr/alias diff. `promotion.patch.diff` teď skládá i konkrétní draft handler stub přímo do `gateway.py`, například `run_<capability>_capability(task_spec, workspace)`, který nese `draft_only` status, `target_capability_name`, `desired_end_state` a recovery text. Pořád je to review-only runtime kandidát, ne auto-applied wiring, ale senior review už dostane patch, který vypadá jako skutečný budoucí hook point místo čistě dokumentačního placeholderu.
...
`GATEWAY_ADMIN_DEPLOY_STATUS` k tomu vrací strukturované diagnostiky `head`, `origin_head`, `upstream`, poslední `before/after` z deploy logu, `deployment_blocker`, `restart_required`, `manual_restart_command`, `sudoers_install_command` a úzký `sudoers_entry` hint. Agent tak nemusí parsovat dlouhý `log_tail`, aby poznal, jestli je problém v git pullu, runtime restartu nebo právech k Dockeru.

Automatický deploy restart se dá povolit úzkým sudoers pravidlem bez otevření obecného sudo. Jednorázově spusť:

    sudo /mnt/c/Repositories/ai-stack/codex/bin/install_deploy_sudoers.sh --install

Helper zapíše jen přesné povolení pro `deploy_ai_stack.sh --restart-only` a `deploy_ai_stack.sh --sudoers-probe`. Stav ověříš:

    /mnt/c/Repositories/ai-stack/codex/bin/install_deploy_sudoers.sh --check

`check_ai_stack.sh` to kontroluje jako `Deploy sudoers NOPASSWD`; defaultně je to jen SKIP s recovery instrukcí, tvrdý fail z toho uděláš přes `REQUIRE_DEPLOY_SUDOERS=1`.
