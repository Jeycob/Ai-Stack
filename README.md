...
- Celkový healthcheck lokálního stacku ve WSL: `bash codex/bin/check_ai_stack.sh`; pro LAN kontrolu nastav `OPENWEBUI_URL=http://192.168.0.48:9090 CODEX_GATEWAY_URL=http://192.168.0.48:9101`. Skript kontroluje i přítomnost WSL boot wrapperu a konfiguraci `/etc/wsl.conf`; chybějící autostart config je jen `SKIP`, protože primární start může dál řešit Windows Task Scheduler. Pokud je dostupný OpenWebUI API key, skript zahrne i audit-chat smoke přes `owui_chat_smoke.py`. Pro levný endpoint-only healthcheck bez modelového smoke použij `SKIP_GATEWAY_SMOKE=1 SKIP_OWUI_CHAT_SMOKE=1 SKIP_OWUI_CHAT_SCENARIOS=1`.
