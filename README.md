...
I tohle už jde přirozeně z OpenWebUI chatu: když uživatel napíše více bodů a otázku typu `Co má dělat jako první?`, `Který úkol je první?`, `Jaký je top task?` nebo `Jen doporuč první krok bez spuštění`, filter to přeloží na `mentor_codex_local.py dispatch --recommend-only` místo plné exekuce.
