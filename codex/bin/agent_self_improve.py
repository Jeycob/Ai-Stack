...
def insert_before_function(text: str, function_name: str, entry_text: str, marker: str) -> str:
    if marker in text:
        return text
    pattern = rf"(?m)^def {re.escape(function_name)}\("
...
def capability_runtime_promotion_patch_text(capability: str, feature_request: str, reasoning: dict[str, Any]) -> str:
    ...
    handler_name = f"run_{capability}_capability"
    handler_text = (
        f"def {handler_name}(task_spec, workspace):\n"
        f'    """Draft runtime hook for `{capability}`.\n\n'
        "    Generated by agent_self_improve as a review-only capability promotion stub.\n"
        '    """\n'
        "    desired_end_state = str((task_spec or {}).get(\"desired_end_state\") or \"\").strip()\n"
        "    target_capability_name = str((task_spec or {}).get(\"target_capability_name\") or \"\").strip()\n"
        "    acceptance_criteria = [\n"
        "        str(item).strip()\n"
        "        for item in ((task_spec or {}).get(\"acceptance_criteria\") or [])\n"
        "        if str(item).strip()\n"
        "    ]\n"
        "    return {\n"
        "        \"ok\": False,\n"
        f"        \"capability\": {capability!r},\n"
        f"        \"workflow\": {runtime_workflow!r},\n"
        "        \"workspace\": workspace,\n"
        f"        \"executor_pattern\": {shape['executor']!r},\n"
        "        \"status\": \"draft_only\",\n"
        "        \"summary\": \"Capability wiring draft exists, but the bounded executor is still pending senior review.\",\n"
        "        \"desired_end_state\": desired_end_state,\n"
        "        \"target_capability_name\": target_capability_name,\n"
        "        \"acceptance_criteria\": acceptance_criteria,\n"
        "        \"recovery\": \"Promote the reviewed executor contract into the live gateway runtime or keep using agent_capability_develop.\",\n"
        "    }\n\n"
    )
    gateway_after = insert_before_function(
        gateway_after,
        "admin_agent_loop",
        handler_text,
        f"def {handler_name}(",
